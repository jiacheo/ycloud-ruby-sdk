...
module YCloudApiClient
  ...
  class WhatsappMessageInteractiveAction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YCloudApiClient::WhatsappMessageInteractiveAction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YCloudApiClient::WhatsappMessageInteractiveAction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@button.nil? && @button.to_s.length > 20
        invalid_properties.push('invalid value for "button", the character length must be smaller than or equal to 20.')
      end

      if !@sections.nil? && @sections.length > 10
        invalid_properties.push('invalid value for "sections", number of items must be less than or equal to 10.')
      end

      if !@sections.nil? && @sections.length < 1
    ...
    def valid?
      ...
      return false if !@button.nil? && @button.to_s.length > 20
      return false if !@sections.nil? && @sections.length > 10
      return false if !@sections.nil? && @sections.length < 1
      true
    end
    ...
    def button=(button)
      if !button.nil? && button.to_s.length > 20
        fail ArgumentError, 'invalid value for "button", the character length must be smaller than or equal to 20.'
      end
